...
<AccordionGroup>
  ...
  <Accordion title="如何为受限模型设置 Hugging Face 令牌？">
    部分 Hugging Face 模型需要你接受许可协议并进行身份验证。设置 `HF_TOKEN` 环境变量：
    ...
    在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取令牌。完整设置步骤见[设置 Hugging Face 令牌](/cn/models/supported#设置-hugging-face-令牌)。
  </Accordion>
