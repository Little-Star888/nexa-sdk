...
"""Pure-Python adb deploy for the QDC Android on-device pytest run.

The QDC appium host is a minimal container with no bash — this module speaks
adb + the Python stdlib only (no ar, no curl). It builds a portable Python
from Termux's aarch64 .deb packages on the runner, pushes pkg-geniex +
bindings + tests/ to the phone, flattens plugin libs, and runs the
device-gated pytest cells in adb shell.
"""
